def CuentaOctameros(GENOMES_PATH,KMERS,MARKOV):
    ...
    output_file = str("_".join (['Markov_count',GENOMES_PATH,date,'OCTANUCS',model,'.txt']))
    ...
    ## Octameros
    if KMERS == "A":
        pals_oct = [Nucs[i]+Nucs[j]+Nucs[k]+Nucs[l]+Nucs[-l]+Nucs[-k]+Nucs[-j]+Nucs[-i] for i in range(1,(len(Nucs))) for j in range(1,(len(Nucs))) for k in range(1,(len(Nucs))) for l in range(1,(len(Nucs)))]
        pals = pals_oct
    ## Octameros parecidos a HIP1
    elif KMERS == "B":
    ...
    try:
        ...
        for GenomeFile in genomes:
            ...
            # k-meros del genoma
            dinuc_genome, trinuc_genome, tetranuc_genome, octanuc_genome = [genome[ii:ii+2] for ii, ch in enumerate(genome)], [genome[ii:ii+3] for ii, ch in enumerate(genome)], [genome[ii:ii+4] for ii, ch in enumerate(genome)], [genome[ii:ii+8] for ii, ch in enumerate(genome)]
            ...
            for pal in pals:
                ...
                OCTANUCLEOTIDES = {octanuc:octanuc_genome.count(octanuc) for octanuc in octanuc_pal}
                ...
                if (MARKOV==0):
                    ...
                    markov = (frec_nuc)*(genome_length - 7)
                    ...
                    # Producto de frecuencias de nucleotidos del palindromo en el genoma (DENOMINADOR)            contador += 1
                    contador_extremos_nuc, denominador_frec_nuc = 1, 1

                    for ii, ch in enumerate(pal):
                        nucleotide = pal[ii+1:ii+2] #pal[ii+1:ii+1+1]
                        if (contador_extremos_nuc <= 6):
                    ...
                    if not (denominador_frec_nuc != 0):
                        ...
                        markov = (numerador_frec_dinuc/denominador_frec_nuc)*(genome_length - 7)
                    ...
                    if not (denominador_frec_dinuc != 0):
                        ...
                        markov = (numerador_frec_trinuc/denominador_frec_dinuc)*(genome_length - 7)
                    ...
                    if not (denominador_frec_trinuc != 0):
                        ...
                        markov = (numerador_frec_tetranuc/denominador_frec_trinuc)*(genome_length - 7)
                        
                else:
                    print("EL GRADO DEL MODELO ES INCORRECTO\n")
                
                output.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(GenomeFileName,pal,OCTANUCLEOTIDES[pal],markov,genome_length,NUCLEOTIDES['A'],NUCLEOTIDES['T'],NUCLEOTIDES['C'],NUCLEOTIDES['G'],NUCLEOTIDES['N']))
